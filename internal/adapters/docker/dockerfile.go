...
// GenerateDockerfileFromBinary creates a minimal Dockerfile for a pre-compiled agent binary.
func GenerateDockerfileFromBinary(binaryPath string, port int) string {
	binaryName := filepath.Base(binaryPath)

	var sb strings.Builder
	sb.WriteString("# Generated by agentspec - do not edit\n")
	sb.WriteString("FROM alpine:3.20\n")
	sb.WriteString("RUN apk add --no-cache ca-certificates\n")
	sb.WriteString("WORKDIR /app\n")
	fmt.Fprintf(&sb, "COPY %s /app/%s\n", binaryName, binaryName)
	fmt.Fprintf(&sb, "RUN chmod +x /app/%s\n", binaryName)
	fmt.Fprintf(&sb, "EXPOSE %d\n", port)
	sb.WriteString("HEALTHCHECK --interval=10s --timeout=3s --start-period=5s --retries=3 \\\n")
	fmt.Fprintf(&sb, "  CMD wget -q --spider http://localhost:%d/healthz || exit 1\n", port)
	fmt.Fprintf(&sb, "ENTRYPOINT [\"/app/%s\"]\n", binaryName)
	fmt.Fprintf(&sb, "CMD [\"--port\", \"%d\"]\n", port)
